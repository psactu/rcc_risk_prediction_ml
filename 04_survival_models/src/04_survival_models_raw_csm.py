...
from uc2_functions import pipeline_skurv
...
def load_importances(path_importances, random_state):
    ...
    features_top_t1 = df_importances[
        (df_importances["random_state"] == random_state)
        & (df_importances["model"] == "RandomSurvivalForest_selector_T1")
    ]["top_features"].tolist()[0]
    features_top_t0 = df_importances[
        (df_importances["random_state"] == random_state)
        & (df_importances["model"] == "RandomSurvivalForest_selector_T0")
    ]["top_features"].tolist()[0]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run survival analysis models with specified configuration."
    )
    parser.add_argument(
        "--RANDOM_STATE",
        default=42,
        type=int,
        help="Random state seed for the simulation.",
    )
    parser.add_argument(
        "--EXPERIMENT_NAME",
    ...
    parser.add_argument(
        "--DATA_ID", default=None, type=str, help="Identifier for the dataset to use."
    )
    parser.add_argument(
        "--PATH_IMPORTANCES",
        default=None,
        type=str,
        help="Path to features selected previously using a RSF on the same seed.",
    )
    parser.add_argument(
        "--N_MAX_FEATURES",
        default=None,
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Path to configuration file for pipeline and search space.",
    )
    parser.add_argument(
        "--DIR_MODEL_PKL",
        default="./models",
...
def select_by_collinearity(
    ...
    highly_correlated_pairs_t1 = get_highly_correlated_pairs(
        df_ohe[features_top_t1], method=method, threshold=threshold
    )
    features_top_t1_filter = filter_features(
        features_top_t1, highly_correlated_pairs_t1
    )

    highly_correlated_pairs_t0 = get_highly_correlated_pairs(
        df_ohe[features_top_t0], method=method, threshold=threshold
    )
    features_top_t0_filter = filter_features(
        features_top_t0, highly_correlated_pairs_t0
    )
    ...
    plot_correlation_matrix(
        features_top_t1_filter,
        "Correlation Matrix of Final Features (T1)",
        "correlation_matrix_final_t1.png",
    )

    # Plot and log the correlation heatmap for final selected features for T0
    plot_correlation_matrix(
        features_top_t0_filter,
        "Correlation Matrix of Final Features (T0)",
        "correlation_matrix_final_t0.png",
    )
...
def main():
    ...
    features_top_t1, features_top_t0 = load_importances(
        args.PATH_IMPORTANCES, int(args.RANDOM_STATE)
    )
    ...
    features_top_t1, features_top_t0 = select_by_collinearity(
        df_ohe=df_ohe,
    ...
    X_train, X_test, y_train, y_test = train_test_split_impute(
        df_ohe=df_ohe, random_state=args.RANDOM_STATE
    )
    models, params = load_config(args.PATH_CONFIG)
    model_instances = [model_mapper(x, args.RANDOM_STATE) for x in models]
    param_grids = params
    assert len(model_instances) == len(param_grids), (
        "The number of model instances and parameter grids must match."
    )
