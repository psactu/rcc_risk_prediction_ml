...
class UnivariateFeatureSelector:
    ...
    def check_if_distributions_are_similar(self, sample1, sample2):
        if sample1 and sample2:  # Add this check
            ...
            if self.verbose:
                print(
                    "Cannot perform Kolmogorov-Smirnov test due to insufficient data."
                )
            return False
...
def plot_feature_importance(
    df, n_features, size: tuple = (5, 8), save_path: str = None
):
    """Plot an histogram given a pandas DataFrame with feature importances.

    Args:
        df: DataFrame with columns 'importances_mean', 'importances_std', 'feature_definition'.
        n_features: Number of top features to show.
        size: Figure size as (width, height) tuple.
        save_path: Optional full file path to save the figure as a vector PDF.
    """
    ...
    # Save as vector PDF if a path is provided
...
def find_problematic_values(values, n_models):
    ...
    problematic_values = [
        value for value, count in value_counts.items() if count != n_models
    ]
...
def delete_run_with_parent(
    experiment, client, least_parent_run_ids_dict, verbose=False
):
        ...
        if "mlflow.parentRunId" in data.tags:
            ...
            if model_path:
                ...
                if random_state in least_parent_run_ids_dict:
                    print(
                        f"random_state {random_state} found in least_parent_run_ids_dict"
                    )
                    if parent_run_id == least_parent_run_ids_dict[random_state]:
                        print(
                            f"Match found for random_state {random_state} and parent_run_id {parent_run_id}"
                        )
                        run_info = client.get_run(run.id).info
                        if run_info.status == "RUNNING":
                            client.set_terminated(run.id)
...
def tune_sksurv_model(
    model, param_grid, k_min, k_step_size, X_tune, y_tune, random_state: int, n_folds=10
):
    ...
    for train_index, val_index in skf.split(X_tune, y_tune["event"]):
        ...
        model_pipeline = Pipeline(
            [("select", SelectKFeaturesTransformer()), ("model", model)]
        )
...
def validate_sksurv_model(
    ...
    ax.axhline(
        mean_auc, linestyle="--", color="red", label=f"Mean AUC = {mean_auc:.3f}"
    )
    ax.set_xlabel("Time $t$ (months)")
...
def pipeline_skurv(
    ...
    best_model, best_score = tune_sksurv_model(
        model=model,
    ...
    model_path = os.path.join(
        dir_models,
        "{}_{}_{}_{}.pkl".format(dataset_name, model_name, timepoint, random_state),
    )
    ...
    result_censored, result_ipcw, score_brier, mean_auc, fig = validate_sksurv_model(
        model=best_model, y_train=y_train, X_test=X_test, y_test=y_test, tau=tau
    )
...
class ModelComparer:
    def __init__(
        self, model1_metrics, model2_metrics, alpha=0.05, higher_is_better=True
    ):
        """
    ...
    def compute_confidence_intervals(self):
        ...
        self.model1_ci = (
            np.percentile(self.model1_metrics, lower_percentile),
            np.percentile(self.model1_metrics, upper_percentile),
        )
        self.model2_ci = (
            np.percentile(self.model2_metrics, lower_percentile),
    ...
    def compute_median_difference(self):
        ...
        upper_percentile = (1 - self.alpha / 2) * 100
        self.ci_diff = (
            np.percentile(differences, lower_percentile),
    ...
    def wilcoxon_signed_rank_test(self):
        ...
        stat, p_value = wilcoxon(
            self.model1_metrics, self.model2_metrics, alternative="two-sided"
        )
    ...
    def print_results(self, model1_name, model2_name):
        ...
        print(
            f"{(1 - self.alpha) * 100}% Confidence Interval of the differences: {self.ci_diff}\n"
        )

        # Wilcoxon signed-rank test
        stat, p_value = self.wilcoxon_signed_rank_test()
        print(
            f"Wilcoxon signed-rank test statistic: {stat:.5f}, p-value: {p_value:.5f}"
        )
        ...
        if p_value < self.alpha:
            if np.median(self.model2_metrics) > np.median(self.model1_metrics):
                ...
                better_model = model1_name if self.higher_is_better else model2_name
            print(
                f"{better_model} is significantly better than the other (based on Wilcoxon signed-rank test)."
            )
        else:
            print(
                "No significant difference between the models based on the Wilcoxon signed-rank test."
            )
    ...
    def plot_performance_distributions(self, model1_name, model2_name):
        ...
        plt.hist(
            self.model1_metrics, bins=20, alpha=0.5, label=model1_name, density=True
        )
        plt.hist(
            self.model2_metrics, bins=20, alpha=0.5, label=model2_name, density=True
        )
    ...
    for run in tqdm(gen):
        ...
        if "mlflow.parentRunId" in data.tags:
            ...
            try:
                l.append(
                    {
                        ...
                        "feature_names_in": data.params["feature_names_in"]
                        if "feature_names_in" in data.params
                        else "",
                        "best_performance_tuning": data.params["best_performance"]
                        if "best_performance" in data.params
                        else "",
                        "concordance_index_censored": data.metrics[
                            "concordance_index_censored"
                        ],
                        "concordance_index_ipcw": data.metrics[
                            "concordance_index_ipcw"
                        ],
                        "integrated_brier_score": data.metrics[
                            "integrated_brier_score"
                        ],
                ...
                parent_run_name = parent_run.data.tags.get(
                    "mlflow.runName"
                )  # Custom tag for run name
                if verbose is True:
                    print(
                        f"Parent run name for model with missing info: {parent_run_name}"
                    )
            ...
            if "RandomSurvivalForest_selector_" in data.tags["mlflow.runName"]:
                try:
                    ...
                    if verbose is True:
                        print(
                            f"Parent run name for model with missing artifact: {parent_run_name}"
                        )
            else:
                continue
    print(len(l))
...
def plot_metric_boxplot(
    ...
    group_map = {
        model: group for group, models in time_groups.items() for model in models
    }
    ...
    order = [
        model for group in time_groups.values() for model in group
    ]  # Flatten the list of models maintaining the group order
    df_metrics_long["order"] = df_metrics_long["model"].apply(
    ...
    hue_order = None
    hue_palette = color_groups if hue_column is None else None
    if hue_column:
        unique_hue_values = sorted(df_metrics[hue_column].unique(), reverse=True)
        hue_order = unique_hue_values
        default_colors = sns.color_palette(n_colors=len(unique_hue_values))
        hue_palette = {
            hue_value: default_colors[i]
    ...
    if metric in [
        ...
        )  # Set yticks from 0.5 to 1 with a step of 0.05
    else:
        ax.set_ylim([0, 0.05])
...
def plot_features_violin(
    ...
    Parameters:
        ...
        save_path (str): If provided, save the figure as a PDF to this path before showing.
    ...
    df_metrics["order"] = pd.Categorical(
        df_metrics["model"], categories=ordered_models, ordered=True
    )
    ...
    violinplot = sns.violinplot(
        ...
        y="n_features_in",
        hue=hue_column if hue_column else "Group",
        palette=color_groups
        if hue_column is None
        else None,  # Use default colors if hue_column is provided
...
def print_metrics(result_censored, result_ipcw, score_brier, mean_auc):
    ...
    print(
        f"Concordance index for right-censored data{' ' * (max_length - len('Concordance index for right-censored data'))} {format(np.round(result_censored, 4), '.4f')}"
    )
    print(
        f"Concordance index for right-censored data based on inverse probability of censoring weights{' ' * (max_length - len('Concordance index for right-censored data based on inverse probability of censoring weights'))} {format(np.round(result_ipcw, 4), '.4f')}"
    )
    print(
        f"Integrated Brier Score (IBS){' ' * (max_length - len('Integrated Brier Score (IBS)'))} {format(np.round(score_brier, 4), '.4f')}"
    )
    print(
        f"Cumulative/dynamic AUC for right-censored time-to-event data{' ' * (max_length - len('Cumulative/dynamic AUC for right-censored time-to-event data'))} {format(np.round(mean_auc, 4), '.4f')}"
    )
